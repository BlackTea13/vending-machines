...
def get_all_vending_machines():
    ...
    return [Vending_Machine.object_to_dictionary(machine) for machine in vending_machines]
...
def get_vending_machine():
    if request.method == 'GET':
        ...
        return Vending_Machine.object_to_dictionary(machine)
...
def create_vending_machine():
    ...
    new_machine = Vending_Machine(form['location'])
...
def add_product_to_machine():
    ...
    listing = Machine_Stock(machine_id, product_id, quantity)
    session.add(listing)
    session.commit()
    session.close()
